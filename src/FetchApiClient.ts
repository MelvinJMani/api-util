...
class FetchApiClient {
	...
	private circuitBreakerTimeout = 30000; // 30 seconds for the circuit breaker
	...
	public addResponseInterceptor(
		interceptor: (data: Response | any) => Promise<any> | any,
	): void {
	...
	private async processQueue() {
		setInterval(async () => {
			...
			if (
				this.requestCount < this.rateLimitCount ||
				timeElapsed >= this.rateLimitInterval
			) {
	...
	private async retryOfflineRequests() {
	...
	private logRequest(url: string, options: RequestInit, response: any) {
		const headers: Record<string, string> = this.convertHeadersToRecord(
			options.headers,
		);
...
function setMockResponse(url: string, response: any) {
	FetchApiClient.prototype.request = async () => response;
}
